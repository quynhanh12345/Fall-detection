...
token = '5749590608:AAEVDrcWtYPQ7XlTqrRn-ZxM4aV03mM3uds'
receiver_id = '5348386070'
        ...
        while cap.isOpened:
            ...
            if ret:
                ...
                for idx in range(output.shape[0]):
                    ...
                    xmin, ymin = (
                        output[idx, 2] - output[idx, 4]/2), (output[idx, 3] - output[idx, 5]/2)
                    xmax, ymax = (
                        output[idx, 2] + output[idx, 4]/2), (output[idx, 3] + output[idx, 5]/2)
                    ...
                    if ((difference > 0) and (int(chest) > thre)) or (difference < 0):
                        ...
                        draw.rounded_rectangle(
                            (cx-10, cy-10, cx+60, cy+60), fill=(84, 61, 247), radius=15)
                        img = np.array(im)
                    ...
                    if (time_start_falling != 0):
                        ...
                        if (time_now - time_start_falling) > 10:
                            bot.sendMessage(
                                receiver_id, "Person Fall Detected")
                            filename = r"falled_img/savedImage.jpg"
                            cv2.imwrite(filename, img)
                            bot.sendPhoto(
                                receiver_id, photo=open(filename, 'rb'))
                            os.remove(filename)
                            time_start_falling = 0
                            time_now = 0
                img_ = img.copy()
                ...
                print(
                    f"Frame {frame_count} Processing. Frames Per Second : {fps}")
                total_fps += fps
                frame_count += 1
                out.write(img)
            else:
...
def Get_coord(kpts, id_part, coord):
    if coord == "x":
        return kpts[3 * id_part]
    if coord == "y":
        return kpts[3 * id_part + 1]
    if coord == "xy":
        return kpts[3 * id_part], kpts[3 * id_part + 1]
